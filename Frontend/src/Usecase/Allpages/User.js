...
function User() {
  ...
  const searchId = useCallback(async () => {
    try {
      const response = await fetch(
        `http://localhost:8000/project_info/${userId}`
      );
  ...
  const searchProject = useCallback(async () => {
    try {
      const response = await fetch(
        `http://localhost:8000/project_info1/${projectTitle}`
      );
  ...
  const saveChanges = async () => {
    try {
      ...
      await axios.put(
        `http://localhost:8000/api/update/${dataToSend.Title}`,
        dataToSend
      );
      setEditMode(false);
      setSuccess(true);
  ...
  const handleTaskButtonClick = async (title) => {
    try {
      const response = await fetch(
        `http://localhost:8000/task_details/${title}`
      );
