...
const Layout = () => {
  ...
  const sendAuditInfo = async (data) => {
    try {
      const response = await axios.post("http://localhost:8000/user/audit", {
        User_id: data.Userid,
      ...
      >
        ...
        >
          ...
          >
            ...
            {imageType === "break" && <img id="secondImg" src={breakImg} alt="" className="logo-wel" />}
            {imageType === "lunch" && <img id="thirdImg" src={lunch} alt="" className="logo-wel" />}
          </Box>
        </Box>
        {/* Right container */}
        <Box
